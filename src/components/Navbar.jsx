		...
		<div ref={navContainerRef} className="fixed inset-x-0 top-4 z-50 h-16 border-none transition-all duration-700 sm:inset-x-6">
			...
			<div className="md:hidden flex justify-end p-4 ">
				<button className="border-2 border-white bg-gray-800 p-2 rounded flex flex-col items-center justify-center" onClick={toggleMenu}>
					<motion.div animate={isMenuOpen ? { rotate: 45, y: 6 } : { rotate: 0, y: 0 }} className="w-6 h-0.5 bg-white mb-1 transition-all" />
					<motion.div animate={isMenuOpen ? { opacity: 0 } : { opacity: 1 }} className="w-6 h-0.5 bg-white mb-1 transition-all" />
					<motion.div animate={isMenuOpen ? { rotate: -45, y: -6 } : { rotate: 0, y: 0 }} className="w-6 h-0.5 bg-white transition-all" />
				</button>
			</div>
